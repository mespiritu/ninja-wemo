...
function Device(app, device, info, devicePollInterval) {
  ...
  setInterval(this.read.bind(this), devicePollInterval || 2000);
 ...
 */
Device.prototype.logReadError = function(err) {
  this.app.log.error('(WeMo) Failed reading device %s - %s', this.name, err.stack.toString());
};
...
Device.prototype.write = function(data) {
  ...
  this.device.setBinaryState(state?1:0, function(err, result) {
    if (err) {
      return this.app.log.error('(WeMo) Failed writing device %s - %s', this.name, err.stack.toString());
    }
